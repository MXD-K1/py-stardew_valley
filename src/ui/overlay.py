...
from data.constants import OVERLAY_POSITIONS
...
class Overlay:
    def __init__(self, player):
        ...
        self.tools_surf = {
            tool: load_image(f"{overlay_path}/{tool}.png").convert_alpha()
            for tool in player.tools
        }
        self.seeds_surf = {
            seed: pygame.transform.scale(
                load_image(f"{overlay_path}/{seed} seed.png"), (50, 50)
            ).convert_alpha()
